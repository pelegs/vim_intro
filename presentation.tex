...
\tikzset{
	key/.style={draw=black, thick, rounded corners=0.5mm, fill=vimgray, inner sep=0pt, minimum width=5mm, minimum height=5mm},
...
\newcommand{\ruleA}[2]{
	\tikz[baseline, node distance=1mm] {
		\node[opr] (opr) {#1};
		\node[mtn, right=of opr] (mtn) {#2};
	}
}
\newcommand{\ruleB}[3]{
	\tikz[baseline] {
		\node[rep] (rep) {#1};
		\node[opr, right=of rep, xshift=-3mm] (opr) {#2};
		\node[mtn, right=of opr] (mtn) {#3};
	}
}
...
\begin{frame}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[node distance=0mm]
			\node[cmd] (cmd) {command};
			\node[right=of cmd] (eq) {=};
			\node[opr, right =of eq] (opr) {operator};
			\node[mtn, right =of opr, xshift=3mm] (mtn) {motion(s)};
		\end{tikzpicture}
	\end{figure}
	...
	\begin{pexample}
		...
		\begin{tabu}[H]{!l^l^l}
			...
			Command & Syntax & Keys \\
			\midrule
			Delete word & \ruleA{del}{word} & \keys{d,w} \\
			Copy until 'A' & \ruleA{copy}{until 'A'} & \keys{y,f,A} \\
			\bottomrule
		\end{tabu}
	\end{pexample}
\end{frame}
